...
public:
    ...
    // Non ancora implementato su Windows (vedi PipeWireEngine per il
    // backend Linux, l'unico su cui è stato realizzato finora) — no-op che
    // segnala l'errore via engineError invece di fallire silenziosamente.
    void setOutputDelayMs(uint32_t sinkNodeId, int delayMs) override;
    // Non ancora implementato su Windows (nessun equivalente diretto del
    ...
    // Non ancora implementato su Windows.
